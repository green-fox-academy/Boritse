...
int main(void) {
	...
	while (1) {
		...
		if (HAL_ADC_PollForConversion(&adc_handle, 10) == HAL_OK) {
			...
			__HAL_TIM_SET_COMPARE(temp_tim_handle, temp_channel, adc_val / 42);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if(button_mode == GREEN){
		button_mode = BLUE;
		temp_tim_handle = &tim5_handle;
		temp_channel = TIM_CHANNEL_4;
	}else if(button_mode == BLUE){
		button_mode = RED;
		temp_tim_handle = &tim3_handle;
		temp_channel = TIM_CHANNEL_1;
	}else{
		button_mode = GREEN;
		temp_tim_handle = &tim2_handle;
		temp_channel = TIM_CHANNEL_1;
	}
}
