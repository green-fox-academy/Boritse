...
int main(int argc, char** argv)
{
	...
	Mat imageHighBrightness;
	image.convertTo(imageHighBrightness, -1, 1, 50); //increase the brightness by 50

	Mat imageLowBrightness;
	image.convertTo(imageLowBrightness, -1, 1, -50); //increase the brightness by 50

	namedWindow("Original", WINDOW_NORMAL);
	namedWindow("HighBrightness", WINDOW_NORMAL);
	namedWindow("LowBrightness", WINDOW_NORMAL);
	
	//Show above images inside the created windows.
	imshow("Original", image);
	imshow("HighBrightness", imageHighContrast);
	imshow("LowBrightness", imageLowContrast);

	resizeWindow("Original", image.cols / 4, image.rows / 4);
	resizeWindow("HighBrightness", image.cols / 4, image.rows / 4);
	resizeWindow("LowBrightness", image.cols / 4, image.rows / 4);
