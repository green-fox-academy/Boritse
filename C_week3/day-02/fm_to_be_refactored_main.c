...
volatile float MHz = 93.1;
volatile float radio_channels[5] = { 88.1, 89.5, 90.9, 92.1, 92.9 };
...
int main(void) {
	...
	while (1) {
		...
		if (MHz == radio_channels[0] || MHz == radio_channels[1]
				|| MHz == radio_channels[2] || MHz == radio_channels[3]
				|| MHz == radio_channels[4]) {
			printf("channel is set to: %.2f\n", MHz);
			HAL_Delay(3000);
		} else {
			printf("currently volume is set to: %d\n", volume);
			HAL_Delay(3000);
		}
		...
		if (switch_up_state == RELEASED) {
			...
			if (time_difference < 1000) {
				...
				if (MHz < radio_channels[0]) {
					...
					MHz = radio_channels[1];
				} else if (MHz >= radio_channels[1]
						&& MHz < radio_channels[2]) {
					MHz = radio_channels[2];
				} else if (MHz >= radio_channels[2]
						&& MHz < radio_channels[3]) {
					MHz = radio_channels[3];
				} else if (MHz >= radio_channels[3]
						&& MHz < radio_channels[4]) {
					MHz = radio_channels[4];
